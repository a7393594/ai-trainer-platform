...
[{{"name": "tool_name", "params": {{...}}}}, ...]"""
    ...
    if not isinstance(plan, list):
        raise ValueError(f"planner did not return a JSON list: {plan_text[:200]}")
    # Empty plan is intentional — the prompt explicitly tells the LLM to return
    # `[]` for chitchat / pure-concept questions where no tools are needed.
    # Downstream gather/loops handle empty list naturally (skip tool exec,
    # synthesizer composes answer from history alone).
...
async def handle_call_model(node: dict, ctx: DAGContext) -> dict:
    ...
    # 上游 capability_* 節點已產出最終回覆 → 跳過主模型(避免覆蓋掉深度多視角分析)。
    # capability_widget / capability_tool_call 設 ctx.capability_handled=True 後,
    # 主模型若再跑一次會用較簡短回覆覆蓋 capability 的完整版本,使用者抱怨
    # "之前都會給我更多的討論方向或是人格模式進行探討" 即此問題。
    # 例外:cfg.allow_after_capability=True 可強制執行(極少數需要二次 polish 的場景)。
    if ctx.capability_handled and not cfg.get("allow_after_capability"):
        _preview = (ctx.llm_response_text or ctx.clean_text or "")[:200]
        ctx.response_metadata.setdefault("call_model_skipped", []).append({
            "node_id": node.get("id"),
            "reason": "capability_handled",
        })
        ctx.emit({
            "type": "node_skipped",
            "node_id": node.get("id"),
        ...
        return {
            ...
            "output": {
                "skipped": True,
                "reason": "capability node already produced final answer",
                "preserved_response_preview": _preview,
            },
            "summary": "已跳過：上游 capability 節點已產出最終回覆",
        }
