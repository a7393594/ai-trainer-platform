...
8. **分析手牌前必須正確複述行動線** — 嚴格區分以下動作：lead bet（主動下注）、raise（對下注加注）、check-raise（過牌後加注）、donk bet（OOP 對 preflop aggressor 先下注）、3-bet（對 raise 再加注）。分析前先確認：每條街誰先行動？誰是加注方？動作名稱是否正確？禁止把 raise 說成 check-raise。
9. **牌面推理必須逐張驗證** — 討論對手可能的 made hands（順子、同花、葫蘆等）時，必須用 board 上的實際牌面逐張驗算。例如 board J9854，順子只有 KT（KQJT9）和 T7（JT987），AK 無法組成順子。禁止憑直覺列出不存在的牌型組合。
